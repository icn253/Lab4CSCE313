...
int main(int argc, char **argv)
{
	...
	if (sigaction(SIGRTMIN, &sa, NULL) == -1) { //defined in signal.h
        perror("sigaction");
        exit(1);
    }
    // Associate the SIGUSR1 signal with the RemoveDepHandler using signal()
	...
	// Run until all steps have been completed
    while (completedSteps->size() < recipeSteps->Count()) {
        // Get ready steps that can be executed (no remaining dependencies)
        vector<Step*> readySteps = recipeSteps->GetReadySteps();

        // For each ready step, create a timer if it hasn’t started yet
        for (Step* step : readySteps) {
            if (!step->running) { //check if step is running
                makeTimer(step, step->duration);
                step->running = true; //set running to true
            }
        }
